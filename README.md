...
- current observed probe mode is `runtime-local-owned`
...
- dense and varlen plain `softcap` backward now route through the exact repo-local runtime replay path, so the seeded backward probe reports `dense_softcap_grad_max_diff=0.0` and `varlen_softcap_grad_max_diff=0.0`
- structured additive-bias varlen `score_mod` backward now also has a native-backed lane: the targeted probe hits `native_varlen_bwd_backend_status()['last_call'] == 'packed_q+packed_k+extra_score_bias+lse_grad'` while still probing exact against the stable Windows shim
